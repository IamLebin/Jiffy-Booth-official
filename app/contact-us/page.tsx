...
export default function ContactPage() {
  ...
  const whatsappDefaultMessage = "Hi Jiffy Booth, I would like to enquire about...";
  ...
  if (loading) return <div className="min-h-screen bg-[#f3f1ee]" />;
    ...
    <main className="min-h-screen flex flex-col bg-[#f3f1ee] text-[#212121] overflow-x-hidden">
      ...
      <section className="w-full bg-[#f3f1ee] py-4 md:py-6 border-b border-gray-100">
        <div className="max-w-6xl mx-auto px-6 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 md:gap-6">

          {/* WhatsApp Card */}
          <a href={`https://wa.me/${data?.whatsappNumber || '60163966562'}?text=${encodeURIComponent(whatsappDefaultMessage)}`} target="_blank" rel="noopener noreferrer"
            className="group bg-white p-5 md:p-6 rounded-2xl shadow-sm hover:shadow-xl hover:-translate-y-2 transition-all duration-300 border border-gray-100 flex flex-row md:flex-col items-center md:text-center gap-4 md:gap-5 min-h-[140px] md:min-h-[190px]">
            ...
            <div className="flex flex-col text-left md:text-center overflow-hidden">
              <h3 className="text-[#1c2431] text-lg md:text-xl font-bold">WhatsApp</h3>
              <p className="text-gray-500 text-xs md:text-sm mt-1 italic">Chat with us instantly</p>
              <div className="mt-2 px-3 py-1.5 bg-gray-50 rounded-full text-[#1c2431] font-bold text-[11px] md:text-sm tracking-tight inline-block w-fit md:w-auto">
          ...
          <a href={`https://instagram.com/${data?.instagramUser || 'jiffybooth'}`} target="_blank" rel="noopener noreferrer"
            ...
            <div className="flex flex-col text-left md:text-center overflow-hidden">
              <h3 className="text-[#1c2431] text-lg md:text-xl font-bold">Instagram</h3>
              <p className="text-gray-500 text-xs md:text-sm mt-1 italic">Instant profile check</p>
              <div className="mt-2 px-3 py-1.5 bg-gray-50 rounded-full text-[#1c2431] font-bold text-[11px] md:text-sm inline-block w-fit md:w-auto">
          ...
          <button
            ...
            className="group bg-white p-5 md:p-6 rounded-2xl shadow-sm hover:shadow-xl hover:-translate-y-2 transition-all duration-300 border border-gray-100 flex flex-row md:flex-col items-center md:text-center gap-4 md:gap-5 min-h-[140px] md:min-h-[190px]"
          >
            <div className="flex-shrink-0 bg-[#9b5744]/10 w-14 h-14 rounded-2xl flex items-center justify-center group-hover:scale-110 transition-transform">
              <svg width="28" height="28" viewBox="0 0 24 24" fill="none" stroke="#9b5744" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
            ...
            <div className="flex flex-col text-left md:text-center overflow-hidden">
              <h3 className="text-[#1c2431] text-lg md:text-xl font-bold">Quotation</h3>
              <p className="text-gray-500 text-xs md:text-sm mt-1 italic">Submit quotation below</p>
              <div className="mt-2 px-3 py-1.5 bg-gray-50 rounded-full text-[#1c2431] font-bold text-[11px] md:text-sm inline-block w-fit md:w-auto">
                Tap to jump to form
              </div>
            </div>
          ...
          <a href={`https://mail.google.com/mail/?view=cm&fs=1&to=${data?.emailAddress || 'hello@jiffybooth.com'}&su=Enquiry for Jiffy Booth`} target="_blank" rel="noopener noreferrer"
            ...
            <div className="flex flex-col text-left md:text-center overflow-hidden">
              <h3 className="text-[#1c2431] text-lg md:text-xl font-bold">Email</h3>
              <p className="text-gray-500 text-xs md:text-sm mt-1 italic">Professional Enquiries</p>
              <div className="mt-2 px-3 py-1.5 bg-gray-50 rounded-full text-[#1c2431] font-bold text-[11px] md:text-sm truncate inline-block w-fit md:w-auto">
      ...
      <section className="max-w-6xl mx-auto py-12 md:py-16 px-3 sm:px-12 lg:px-16">
        <div className="flex flex-col lg:flex-row gap-12 lg:gap-20 items-start">
          ...
          <div id="contact-form" ref={formSectionRef} className="flex-1 lg:max-w-xl scroll-mt-24">
            <EnquiryForm whatsappNumber={data?.whatsappNumber || '60163966562'} isDarkBackground={true} />
          </div>
        </div>
      </section>
    </main>
